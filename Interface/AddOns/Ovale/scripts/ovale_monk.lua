...
do
	...
	local desc = "[6.2] SimulationCraft: Monk_Brewmaster_1h_CE_T17M"
	local code = [[
...
AddFunction BrewmasterAoeMainActions
{
	...
	#rushing_jade_wind,if=chi.max-chi>=1&buff.serenity.down
	if MaxChi() - Chi() >= 1 and BuffExpires(serenity_buff) Spell(rushing_jade_wind)
...
AddFunction BrewmasterAoeShortCdActions
{
	...
	unless BuffExpires(shuffle_buff) and Spell(blackout_kick)
	{
		...
		unless Chi() >= 4 and Spell(chi_explosion_tank)
		{
			...
			unless MaxChi() - Chi() >= 2 and not BuffPresent(serenity_buff) and Spell(keg_smash) or BuffRemaining(shuffle_buff) <= 3 and SpellCooldown(keg_smash) >= GCD() and Spell(blackout_kick) or BuffPresent(serenity_buff) and Spell(blackout_kick) or MaxChi() - Chi() >= 1 and BuffExpires(serenity_buff) and Spell(rushing_jade_wind)
			{
...
AddFunction BrewmasterStMainActions
{
	...
	#chi_brew,if=target.health.percent<10&cooldown.touch_of_death.remains=0&chi.max-chi>=2&(buff.shuffle.remains>=6|target.time_to_die<buff.shuffle.remains)&!glyph.touch_of_death.enabled
	if target.HealthPercent() < 10 and not SpellCooldown(touch_of_death) > 0 and MaxChi() - Chi() >= 2 and { BuffRemaining(shuffle_buff) >= 6 or target.TimeToDie() < BuffRemaining(shuffle_buff) } and not Glyph(glyph_of_touch_of_death) Spell(chi_brew)
	#keg_smash,if=chi.max-chi>=2&!buff.serenity.remains
	if MaxChi() - Chi() >= 2 and not BuffPresent(serenity_buff) Spell(keg_smash)
	#blackout_kick,if=buff.shuffle.remains<=3&cooldown.keg_smash.remains>=gcd
	if BuffRemaining(shuffle_buff) <= 3 and SpellCooldown(keg_smash) >= GCD() Spell(blackout_kick)
...
AddFunction BrewmasterStShortCdActions
{
	...
	unless BuffExpires(shuffle_buff) and Spell(blackout_kick)
	{
		...
		unless Chi() >= 3 and Spell(chi_explosion_tank)
		{
			...
			unless MaxChi() - Chi() >= 2 and not BuffPresent(serenity_buff) and Spell(keg_smash) or BuffRemaining(shuffle_buff) <= 3 and SpellCooldown(keg_smash) >= GCD() and Spell(blackout_kick) or BuffPresent(serenity_buff) and Spell(blackout_kick)
			{
				#chi_burst,if=energy.time_to_max>2&buff.serenity.down
				if TimeToMaxEnergy() > 2 and BuffExpires(serenity_buff) and CheckBoxOn(opt_chi_burst) Spell(chi_burst)
			}
...
{
	BuffExpires(shuffle_buff) and Spell(blackout_kick) or Chi() >= 3 and Spell(chi_explosion_tank) or MaxChi() - Chi() >= 2 and not BuffPresent(serenity_buff) and Spell(keg_smash) or BuffRemaining(shuffle_buff) <= 3 and SpellCooldown(keg_smash) >= GCD() and Spell(blackout_kick) or BuffPresent(serenity_buff) and Spell(blackout_kick) or TimeToMaxEnergy() > 2 and BuffExpires(serenity_buff) and Spell(chi_wave) or not BuffPresent(zen_sphere_buff) and TimeToMaxEnergy() > 2 and BuffExpires(serenity_buff) and Spell(zen_sphere) or MaxChi() - Chi() < 2 and Spell(blackout_kick) or MaxChi() - Chi() >= 1 and SpellCooldown(keg_smash) >= GCD() and Energy() + EnergyRegenRate() * SpellCooldown(keg_smash) >= 80 and Spell(expel_harm) or MaxChi() - Chi() >= 1 and SpellCooldown(keg_smash) >= GCD() and SpellCooldown(expel_harm) >= GCD() and Energy() + EnergyRegenRate() * SpellCooldown(keg_smash) >= 80 and Spell(jab) or Spell(tiger_palm)
}
...
do
	local name = "simulationcraft_monk_brewmaster_1h_serenity_t17m"
	local desc = "[6.2] SimulationCraft: Monk_Brewmaster_1h_Serenity_T17M"
	local code = [[
...
do
	local name = "simulationcraft_monk_brewmaster_2h_ce_t17m"
	local desc = "[6.2] SimulationCraft: Monk_Brewmaster_2h_CE_T17M"
	local code = [[
...
do
	...
	local desc = "[6.2] SimulationCraft: Monk_Brewmaster_2h_Serenity_T17M"
	local code = [[
...
AddFunction WindwalkerDefaultMainActions
{
	...
	if not Talent(chi_explosion_talent) and BuffRemaining(tiger_power_buff) < 6.6 Spell(tiger_palm)
	...
	if Talent(chi_explosion_talent) and { SpellCooldown(fists_of_fury) < 5 or not SpellCooldown(fists_of_fury) > 0 } and BuffRemaining(tiger_power_buff) < 5 Spell(tiger_palm)
...
AddFunction WindwalkerDefaultShortCdActions
{
	unless target.DebuffExpires(storm_earth_and_fire_target_debuff) and CheckBoxOn(opt_storm_earth_and_fire) and Enemies() > 1 and { Enemies() < 3 and BuffStacks(storm_earth_and_fire_buff) < 1 or Enemies() >= 3 and BuffStacks(storm_earth_and_fire_buff) < 2 } and Spell(storm_earth_and_fire text=other) or target.DebuffExpires(storm_earth_and_fire_target_debuff) and CheckBoxOn(opt_storm_earth_and_fire) and Enemies() > 1 and { Enemies() < 3 and BuffStacks(storm_earth_and_fire_buff) < 1 or Enemies() >= 3 and BuffStacks(storm_earth_and_fire_buff) < 2 } and Spell(storm_earth_and_fire text=3)
	{
		...
		unless Talent(serenity_talent) and Talent(chi_brew_talent) and not SpellCooldown(fists_of_fury) > 0 and TimeInCombat() < 20 and WindwalkerOpenerShortCdPostConditions() or not Talent(chi_explosion_talent) and BuffRemaining(tiger_power_buff) < 6.6 and Spell(tiger_palm) or Talent(chi_explosion_talent) and { SpellCooldown(fists_of_fury) < 5 or not SpellCooldown(fists_of_fury) > 0 } and BuffRemaining(tiger_power_buff) < 5 and Spell(tiger_palm)
		{
			...
			unless { target.DebuffExpires(rising_sun_kick_debuff) or target.DebuffRemaining(rising_sun_kick_debuff) < 3 } and Spell(rising_sun_kick)
			{
				...
				unless BuffRemaining(tiger_power_buff) > CastTime(fists_of_fury) and target.DebuffRemaining(rising_sun_kick_debuff) > CastTime(fists_of_fury) and TimeToMaxEnergy() > CastTime(fists_of_fury) and not BuffPresent(serenity_buff) and Spell(fists_of_fury)
				{
					...
					if SpellCooldown(fists_of_fury) > 6 and { not Talent(serenity_talent) or not BuffPresent(serenity_buff) and SpellCooldown(serenity) > 4 } and Energy() + EnergyRegenRate() < 50 Spell(energizing_brew)
					#call_action_list,name=st,if=active_enemies<3&(level<100|!talent.chi_explosion.enabled)
					...
					unless Enemies() < 3 and { Level() < 100 or not Talent(chi_explosion_talent) } and WindwalkerStShortCdPostConditions()
					{
						...
						unless Enemies() == 1 and Talent(chi_explosion_talent) and WindwalkerStChixShortCdPostConditions()
						{
							...
							unless { Enemies() == 2 or Enemies() == 3 and not Talent(rushing_jade_wind_talent) } and Talent(chi_explosion_talent) and WindwalkerCleaveChixShortCdPostConditions()
							{
								...
								unless Enemies() >= 3 and not Talent(rushing_jade_wind_talent) and not Talent(chi_explosion_talent) and WindwalkerAoeNorjwShortCdPostConditions()
								{
									#call_action_list,name=aoe_norjw_chix,if=active_enemies>=4&!talent.rushing_jade_wind.enabled&talent.chi_explosion.enabled
									if Enemies() >= 4 and not Talent(rushing_jade_wind_talent) and Talent(chi_explosion_talent) WindwalkerAoeNorjwChixShortCdActions()

									unless Enemies() >= 4 and not Talent(rushing_jade_wind_talent) and Talent(chi_explosion_talent) and WindwalkerAoeNorjwChixShortCdPostConditions()
									{
										#call_action_list,name=aoe_rjw,if=active_enemies>=3&talent.rushing_jade_wind.enabled
...
AddFunction WindwalkerAoeNorjwMainActions
{
	#chi_wave,if=energy.time_to_max>2&buff.serenity.down
	if TimeToMaxEnergy() > 2 and BuffExpires(serenity_buff) Spell(chi_wave)
	...
	if MaxChi() - Chi() < 2 and SpellCooldown(fists_of_fury) > 3 Spell(blackout_kick)
	#spinning_crane_kick
	Spell(spinning_crane_kick)
}

AddFunction WindwalkerAoeNorjwShortCdActions
{
	unless TimeToMaxEnergy() > 2 and BuffExpires(serenity_buff) and Spell(chi_wave)
	{
		#chi_burst,if=energy.time_to_max>2&buff.serenity.down
		if TimeToMaxEnergy() > 2 and BuffExpires(serenity_buff) and CheckBoxOn(opt_chi_burst) Spell(chi_burst)
...
AddFunction WindwalkerAoeNorjwShortCdPostConditions
{
	TimeToMaxEnergy() > 2 and BuffExpires(serenity_buff) and Spell(chi_wave) or TimeToMaxEnergy() > 2 and not BuffPresent(zen_sphere_buff) and BuffExpires(serenity_buff) and Spell(zen_sphere) or { BuffPresent(combo_breaker_bok_buff) or BuffPresent(serenity_buff) } and Spell(blackout_kick) or BuffPresent(combo_breaker_tp_buff) and BuffRemaining(combo_breaker_tp_buff) <= 2 and Spell(tiger_palm) or MaxChi() - Chi() < 2 and SpellCooldown(fists_of_fury) > 3 and Spell(blackout_kick) or Spell(spinning_crane_kick)
}

### actions.aoe_norjw_chix

AddFunction WindwalkerAoeNorjwChixMainActions
{
...
AddFunction WindwalkerAoeNorjwChixShortCdActions
{
	unless Chi() >= 4 and SpellCooldown(fists_of_fury) > 4 and Spell(chi_explosion_melee) or Chi() == MaxChi() and Spell(rising_sun_kick) or TimeToMaxEnergy() > 2 and Spell(chi_wave)
	{
...
AddFunction WindwalkerAoeNorjwChixShortCdPostConditions
{
	Chi() >= 4 and SpellCooldown(fists_of_fury) > 4 and Spell(chi_explosion_melee) or Chi() == MaxChi() and Spell(rising_sun_kick) or TimeToMaxEnergy() > 2 and Spell(chi_wave) or TimeToMaxEnergy() > 2 and not BuffPresent(zen_sphere_buff) and Spell(zen_sphere) or Spell(spinning_crane_kick)
}
...
{
	unless Chi() >= 2 and BuffPresent(combo_breaker_ce_buff) and SpellCooldown(fists_of_fury) > 2 and Spell(chi_explosion_melee) or BuffPresent(combo_breaker_tp_buff) and BuffRemaining(combo_breaker_tp_buff) <= 2 and Spell(tiger_palm) or Spell(rising_sun_kick) or TimeToMaxEnergy() > 2 and Spell(chi_wave)
	{
		#chi_burst,if=energy.time_to_max>2
		if TimeToMaxEnergy() > 2 and CheckBoxOn(opt_chi_burst) Spell(chi_burst)
...
do
	local name = "simulationcraft_monk_windwalker_2h_t18m"
	local desc = "[6.2] SimulationCraft: Monk_Windwalker_2h_T18M"
	local code = [[
# Based on SimulationCraft profile "Monk_Windwalker_2h_T18M".
#	class=monk
#	spec=windwalker
#	talents=0230023
#	glyphs=touch_of_death,blackout_kick
