...
function oq.editbox( parent, name, x, y, cx, cy, max_chars, func, init_val )
  ...
  e.str = init_val or "" ;
  e.func = func ;
  e:SetScript( "OnTextChanged"  , function(self) self.str = self:GetText() or "" ; if (self.func ~= nil) then self.func(self, self.str) ; end end ) ;
  e:SetScript( "OnEscapePressed", function(self) self:ClearFocus() end ) ;
  e:SetText(init_val or "" ) ;
  if (oq.__backdrop17 == nil) then
    oq.__backdrop17 = { bgFile="Interface/Tooltips/UI-Tooltip-Background", 
...
function oq.menu_create()
  if (oq.__menu == nil) then
    ...
    if (oq.__backdrop18 == nil) then
      oq.__backdrop18 = { bgFile="Interface/Tooltips/CHATBUBBLE-BACKGROUND", 
                          edgeFile="Interface/Tooltips/UI-Tooltip-Border", 
                          tile=true, tileSize = 16, edgeSize = 16,
...
function oq.button_pulldown( parent, x, y, cx, cy, populate_list_func, init_texture ) 
  ...
  cb:SetScript( "OnClick", function(self,button) 
                             if (oq.menu_is_visible()) then
                               ...
                               oq.menu_show( self._selected, "TOPLEFT", 0, -25, "BOTTOMLEFT", self._selected:GetWidth()*2 + (self.__width or 130) ) ;
                             end
                           end 
              ) ;
  return cb ;
...
function oq.pushbutton_pulldown( parent, x, y, cx, cy, populate_list_func, init_texture ) 
  local cb = oq.texture_button( parent, x, y, cx, cy, init_texture, init_texture, nil, 
