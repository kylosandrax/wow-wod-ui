...
function core:UpdateCommunicationsStatus()
	...
	if initSelfSender == true then
		...
		Player.info = {} 
		Player.info.realm = Player.realm
...
function core:UpdateSenderStatus(sender, allowEvents)
	...
	if Player.raid then
		...
		if raidId then
			...
			if (sender.realm) then
				senderName = senderName .. "-" .. sender.realm
				name = name .. "-" .. sender.realm
			end
	...
	elseif Player.party then
		if UnitInParty(sender.name) then
			...
			sender.dead = UnitIsDead(sender.name) or UnitIsGhost(sender.name)
		else
...
function core:ProcessPlayer(guid, info)
	...
	if info.realm then
		player.name = info.name .. "-" .. info.realm
	end
...
function core:GetUnitFromName(name)
	...
	if IsInRaid() then
		...
		for i = 1, numRaidMembers do
			local unit = "raid"..i
			local realName, realm = UnitName(unit)
			local fullName = realName
			if realm then fullName = realName .. "-" .. realm end
	...
	elseif UnitInParty("player") then
		...
		for i = 1, numPartyMembers do
			...
			local fullName = realName
			if realm then fullName = realName .. "-" .. realm end
			-- print("fullName/name: ", fullName, name)
			if fullName == name then
				return unit
			end
		end
	...
	optionTables.args.Spells.args.Create = {
		...
		args = {
			...
			talentSpecDropDown = {
				...
				get = function(info) return newRequirementTemplate.specialization end,
				set = function(info, value)
			...
			spacer2A = {
				...
				order = 18,
				fontSize = "medium",
				hidden = not (tablelength(_talentNameKeys) == 0 and REQUIREMENT_KEYS[newRequirementTemplate.type] == REQUIREMENT_KEYS.TALENT_NAME),
			},
			spacer2B = {
				type = "description",
				name = "",
