...
f1.stringHeader:SetPoint("BOTTOM",f1,"TOP",0,-40)
...
f1.scrollframe.stringAbout:SetSize(f1.scrollframe:GetWidth(), 30)
...
local function MakeScrollObject(index)
	...
	button:SetScript("OnClick", function(btn)
		...
		f1.editFrame.editBoxName:SetText(list[index].name)
		f1.editFrame.editBoxWords.EditBox:SetText(list[index].words)
	...
	frame:SetScript("OnClick", function(btn)
		hopAddon.searchFrame.searchBox:SetText(list[index].words)
...
local function UpdateFrames()
	...
	for i=1,#list do
		f1.scrollframe.scrollchild.frames[i].stringName:SetText(list[i].name)
		f1.scrollframe.scrollchild.frames[i].stringKeywords:SetText(list[i].words)
...
f1.editFrame.buttonSave:SetScript("OnClick",function(btn)
	if f1.editFrame.index == 0 then
		...
		table.insert(list,t)
		if #list > #f1.scrollframe.scrollchild.frames then
			MakeScrollObject(#list)
		else
			UpdateFrames()
		end
	else
		list[f1.editFrame.index].name = f1.editFrame.editBoxName:GetText()
		list[f1.editFrame.index].words = f1.editFrame.editBoxWords.EditBox:GetText()
		UpdateFrames()
	end
	f1.editFrame:Hide()	
	f1.scrollframe:Show()
...
f1.editFrame.buttonDelete:SetScript("OnClick",function(btn)
	table.remove(list,f1.editFrame.index)
	UpdateFrames()
	f1.editFrame:Hide()
	f1.scrollframe:Show()
end)
...
local function MakeEntryMenu()
	...
	for	i=1,#list do
		...
		entry.text = list[i].name
		entry.func = function()
			hopAddon.searchFrame.searchBox:SetText(list[i].words)
			hopAddon_searchBox_OnEnter(hopAddon.searchFrame.searchBox)
			PlaySound("igMainMenuOptionCheckBoxOn")
		end
...
hopAddon.searchFrame.buttonFavourites:SetScript("OnClick", function(self, button)
	if button == "LeftButton" then
		if f1:IsShown() then
			...
			if not init then
				for i=1,#list do MakeScrollObject(i) end
				init = true
			end
			f1:Show()
		end
...
local function Favourites_EventSystem(self,event,...)
	if event == "PLAYER_LOGIN" then	
		if ServerHopFavouritesList ~= nil then
			list = ServerHopFavouritesList
		end
	elseif event == "PLAYER_LOGOUT" then
		local t = list
		ServerHopFavouritesList = {}
		ServerHopFavouritesList = t
	end

end

hopAddon.searchFrame.buttonFavourites:SetScript("OnEvent", Favourites_EventSystem)
hopAddon.searchFrame.buttonFavourites:RegisterEvent("PLAYER_LOGIN")
hopAddon.searchFrame.buttonFavourites:RegisterEvent("PLAYER_LOGOUT")
