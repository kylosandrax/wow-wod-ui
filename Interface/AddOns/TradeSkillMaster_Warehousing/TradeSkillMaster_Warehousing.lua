...
function TSM:GetItem(args)
	if args then
		if TSM.move:areBanksVisible() then
			...
			if not searchString and TSMAPI.Item:GetInfo(quantity) then -- incase an itemID was entered but no qty then the strmatch would have incorrectly set as quantity
